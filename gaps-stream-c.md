...
|-------|--------|-------|--------|
| **C9a** | `pending_orders_placed_label.dart`, `pending_orders_view_data.dart`, `pending_orders_mock_data.dart` — sort; **`PendingOrdersFilterState`** multi-select **Buy + Sell** (no “all”; both = all directions); **game titles** multi-select (**empty selection = all games**). | `pending_orders_placed_label_test.dart`, `pending_orders_view_data_test.dart` | `applyPendingOrdersFilters`. |
...
| **C9c** | `PendingOrdersScreen` — filter sheet **`FilterChip`** + **Apply/Reset**; `onCancelOrder?` stub. | `pending_orders_screen_test.dart` | |
...
| 2026-05-10 | **Trading → results auto-route** | — | `GameTradingRouteScreen` listens to `currentGameProvider` and navigates to results when `game_state` is `trading_ended` / `game_finalised` / `discarded`; router tests seed `currentGame` stubs; `game_trading_route_screen_test` covers transition. |
...
| **Trading → Results navigation** | **Addressed (2026-05-10):** `GameTradingRouteScreen` watches `currentGameProvider` and **`go`**s to `/game/:id/results` when `game_state` is `trading_ended`, `game_finalised`, or `discarded` (deferred via `Future.microtask`). | Optional: snackbar “View results” if product prefers manual CTA instead of auto-route. |
