...
| **Sign-out and delete-account are stubs** | **`onSignOut: () {}`**; **`onDeleteAccount`** not passed from router (dialog confirm is a no-op). | **`onSignOut`:** revoke session + **`context.go(AppRoutes.auth)`**. **`onDeleteAccount`:** account deletion RPC + clear local session + navigate. |
...
| **Profile (`/profile`)** | **`ProfileScreen`** replaces prior placeholder (C8 slice): sticky header back + centred title (no shell profile icon); mock `ProfileViewData`; username row with pencil → **dual actions while editing**: **cancel** restores committed value / **confirm** submits (disabled until text differs); **`Username taken`** inline when rename returns `taken`; bottom nav escapes to shell routes. **`/history`** still placeholder. Router: mock `onUsernameCommit`, **`onSignOut` noop**, **`onDeleteAccount`** omitted. | Load profile from Supabase/session; real rename + uniqueness errors; **`onSignOut`** clears session + route to **`/auth`**; **`onDeleteAccount`** destructive API; ship **History** UI; reconcile with Streams A/B auth. |
