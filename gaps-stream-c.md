...
| 2026-04-23 | **C6** PnL calculator (trading) | `feat(trading): C6 PnL calculator with dynamic envelope bounds` | `projectedPnl`, `envelopeSliderBoundsForCenter` / `valueFitsInBounds`, input parsing, `PnlCalculator` (ExpansionTile, slider + typed + reset, dynamic bounds), `formatProjectedPnl`, `GameTradingScreen` order **stats → PnL → chart → order book**; full `flutter test` green, `dart analyze` unchanged warnings elsewhere. **Browser MCP** for `/game/g1/trading` not run in this session. **Bounds example:** `v=384` → **$190–$580** (per implemented tier + rounding); re-open if the product target was different (e.g. $100–$600 for that scenario). |
